...
public class GameFrame extends JFrame{
	...
	/**Referencia a játéktábla adataira*/
	private FieldData field_data;
	/**Referencia a bábutábla adataira*/
	private PawnData pawn_data;
	/**Referencia a táblán található sziklák adataira*/
	...
	/**
	 * Betölti folytatásra a fájlba elmentett játékot.
	 * */
	...
	private class PushActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(board.getSelectedColumn()>=0  ) {
				...
				//Ha van azon álló tárgy ÉS az egy bábu ÉS az a játékosé ÉS a lökés irányában van valami
				if((p = (Pawn)((Field)board.getValueAt(board.getSelectedRow(), board.getSelectedColumn())).GetThing()) != null && p.toString().equals("Pawn") && 
						((player1_turn && p.getIcon().toString().equals("E")) || (!player1_turn && p.getIcon().toString().equals("R"))) &&
					...
					if(leesett_babu_e != null) {
						...
						//Más esetben, egy szikla lehetett csak, ekkor a játéknak vége, és a játékos nyert
						else {
						
						
							//Nyertes eldöntése: aki az utolsó a sorban
							Field utolso_a_sorban = p.GetField();
							//Utolsó sorban lévõ elem
							while(utolso_a_sorban.GetNeighbor(p.GetFacing()) != null) {
								utolso_a_sorban = utolso_a_sorban.GetNeighbor(p.GetFacing());
							}
							//Addig megyünk vissza, amíg meg nem találjuk a nyertest: Bábu ÉS a lökés irányába nézett
							Pawn p_nyertes = p;
							...
							while(utolso_a_sorban.GetNeighbor(ellentetes_irany) != p.GetField().GetNeighbor(ellentetes_irany)) {
								...
								// Ha az adott mezõn bábu van, és a lökés irányába néz, akkor meg van a nyertes bábu
					...
					JOptionPane.showMessageDialog(new JFrame(), "Ez egy üres mezõ vagy nem saját bábut választottál, avagy nincs mit lökni!", "Hiba lökéskor", JOptionPane.ERROR_MESSAGE);
			}else
