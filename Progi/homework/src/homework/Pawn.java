...
public class Pawn extends Thing{
	...
	//----------ADATTAGOK----------
	/**Megadja, hogy adott bábu éppen melyik irányba néz.*/
	...
	private int calculatePush() {
		...
		while(iterator != null && iterator.GetThing() != null) {			//Addig lépunk, amíg van szomszédos mezõ adott irányba ÉS azon van vmi
	 ...
	 * @return Visszatéríti a leesett tárgyat a tábláról.
	 * */
	public Thing Push() {
		...
		if(calculatePush() > 0) {
			...
			while(iterator != innen_indultam.GetNeighbor(facing.Ellentet(facing))) {
				...
				if(t1 != null) {														//Megvizsgáljuk, hogy van-e az aktuális mezõn tolható tárgy ...
					...
					else if(szomszed_iranyban == null && t1.toString().equals("Rock"))	// ... HA NEM LÉTEZIK ÉS A TÁRGY EGY SZIKLA, akkor visszatérítjük, mint leesett tárgyat a tábláról.
